...
source_path = f"s3://{args['source_bucket_name']}/{source_prefix}/"
...
# ------------------------------------------------------------
# Log the raw DataFrame schema before transformations.
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Log a sample of the raw DataFrame before transformations.
# ------------------------------------------------------------
if raw_count > 0:
...
if raw_count == 0:
    ...
    # --------------------------------------------------------
    # Log the schema after source_file has been added.
    # --------------------------------------------------------
    print("===== SCHEMA AFTER ADDING SOURCE_FILE =====")
    df.printSchema()


    # --------------------------------------------------------
    # Log the distinct source files included in this run.
    #
    # The value 100 only limits the displayed log output.
    # It does not limit the number of files processed.
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Log how the source paths were translated into partition
    # values.
    # --------------------------------------------------------
    print("===== EXTRACTED PARTITION VALUES =====")
    ...
    # --------------------------------------------------------
    # Log the file paths that failed partition extraction.
    # --------------------------------------------------------
    if invalid_partition_count > 0:
        print("===== FILES WITH INVALID PARTITION PATHS =====")

        invalid_partition_df.select(
            "source_file",
            "year",
            "month",
            "day"
    ...
    # --------------------------------------------------------
    # Log the schema before complex columns are converted.
    # --------------------------------------------------------
    print("===== SCHEMA BEFORE COMPLEX COLUMN CONVERSION =====")
    df.printSchema()
    ...
    # --------------------------------------------------------
    # Log which fields were converted.
    # --------------------------------------------------------
    if complex_columns:
        print(
        ...
        print(
            "No StructType, ArrayType or MapType columns "
            "required conversion."
        )


    # --------------------------------------------------------
    # Log the schema after complex fields are converted.
    #
    # Converted complex fields should now appear as strings.
    # --------------------------------------------------------
    print("===== SCHEMA AFTER COMPLEX COLUMN CONVERSION =====")
    df.printSchema()
    ...
    # --------------------------------------------------------
    # Log the final schema that will be written to Parquet.
    # --------------------------------------------------------
    ...
    if final_count == 0:
        ...
        print("===== FINAL DATAFRAME SAMPLE BEFORE WRITE =====")
