...
all_target_columns = [
    ...
    # Appended to the existing Redshift tables with ALTER TABLE. Keep these
    # last so the DataFrame order matches the physical Redshift column order.
...
# Restrict incremental discovery to recent hourly partitions. The Glue job
# bookmark still prevents files already processed within these partitions
# from being processed again. Initial loads continue to read all history.
PARTITION_LOOKBACK_HOURS = int(
    get_optional_job_arg(
        "partition_lookback_hours",
        "24",
    )
)

if PARTITION_LOOKBACK_HOURS < 1:
    raise ValueError(
        "--partition_lookback_hours must be at least 1"
    )
...
partition_hours = [
    ...
    for offset in range(PARTITION_LOOKBACK_HOURS + 1)
]
...
if INITIAL_LOAD:
    ...
    print(
        "Incremental partition lookback: "
        f"{PARTITION_LOOKBACK_HOURS} hours"
    )
...
def value_or_null(
    ...
    ).otherwise(source_value.cast(spark_type))
...
def timestamp_or_null(
    ...
    source_text = trim(col(actual_path).cast("string"))

    return when(
        col(actual_path).isNull() | (source_text == ""),
        lit(None).cast("timestamp"),
    ).otherwise(
        from_utc_timestamp(
            to_timestamp(source_text),
            "Africa/Johannesburg",
        )
    )


def text_or_null(value_expression):
    """Return trimmed text, or NULL for null/blank/whitespace input."""
    source_text = trim(value_expression.cast("string"))

    return when(
        value_expression.isNull() | (source_text == ""),
        lit(None).cast("string"),
    ).otherwise(source_text)
...
def json_text_or_null(
    ...
    return text_or_null(col(actual_path))
...
if DEBUG_CONTACT_ID:
    print(
        "DEBUG â€” source rows before flattening for ContactId "
        f"{DEBUG_CONTACT_ID}:"
    )
...
if DEBUG_CONTACT_ID:
    print(
        "DEBUG â€” flattened rows before in-batch deduplication for "
...
if DEBUG_CONTACT_ID:
    print(
        "DEBUG â€” flattened rows after exact-version deduplication for "
        f"ContactId {DEBUG_CONTACT_ID}:"
    )
...
if LATEST_ONLY:
    ...
    if DEBUG_CONTACT_ID:
        print(
            "DEBUG â€” latest selected row for ContactId "
            f"{DEBUG_CONTACT_ID}:"
        )
...
cached_flattened_df = flattened_df.persist(
    StorageLevel.MEMORY_AND_DISK
)
flattened_df = cached_flattened_df

row_count = cached_flattened_df.count()
...
if row_count == 0:
    ...
    # ============================================================
    # Redshift MERGE upsert
    #
    # - Truncate staging before each run.
    # - Keep one target row per contact_id.
    # - Update an existing contact_id only when the incoming
    #   last_update_timestamp is newer.
    # - Insert a contact_id that does not exist in the target.
    # - Never delete rows from the target.
    # ============================================================
    ...
    # Amazon Redshift does not support a search condition directly on
    # WHEN MATCHED. Apply the newer-timestamp check to every assignment
    # instead, preventing an older or identical source row from replacing
    # the current target values.
    update_assignments = ",\n        ".join(
        (
            f'"{column_name}" = CASE '
            f'WHEN "{TARGET_TABLE}"."last_update_timestamp" IS NULL '
            f'OR ('
            f'source."last_update_timestamp" IS NOT NULL AND '
            f'source."last_update_timestamp" > '
            f'"{TARGET_TABLE}"."last_update_timestamp"'
            f') '
            f'THEN source."{column_name}" '
            f'ELSE "{TARGET_TABLE}"."{column_name}" END'
        )
        for column_name in all_target_columns
    ...
    MERGE INTO {QUALIFIED_TARGET}
    USING {QUALIFIED_STAGING} AS source
       ON "{TARGET_TABLE}"."contact_id" = source."contact_id"
    WHEN MATCHED THEN
        UPDATE SET
        {update_assignments}
    ...
    # ============================================================
    # Write to same-schema staging table, then upsert into target
    # ============================================================
